...
int map_count = 0;
extern pthread_mutex_t mutex_for_vector;
              ...
              int flags, int fd, off_t offset)
{
  ...
  // DLOG(ERROR, "ret = %p, length = 0x%zx, prot = %d, flags = %d, fd = %d, offset = 0x%zx\n", ret, length, prot, flags, fd, offset);
  if (ret != MAP_FAILED) {
    addRegionTommaps(ret, length);
    // DLOG(NOISE, "LH: mmap (%lu): %p @ 0x%zx\n", mmaps.size(), ret, length);
    if (fd > 0) {
      char glibcFullPath[PATH_MAX] = {0};
      int found = checkLibrary(fd, "libc-2.31.so", glibcFullPath, PATH_MAX);
...
__munmapWrapper(void *addr, size_t length)
{
  ...
  if (ret != -1) {
    pthread_mutex_lock(&mutex_for_vector);
    updateMmaps(addr, length);
    DLOG(4, "LH: munmap (%lu): %p @ 0x%zx\n", mmaps.size(), addr, length);
    pthread_mutex_unlock(&mutex_for_vector);
  }
  return ret;
}
