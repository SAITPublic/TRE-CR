...
header_file.write("#include <cufft.h>\n")
...
stub_lib.write("#include <cufft.h>\n\n")
# =============================================================
...
def emit_wrapper(decl, ret_type, fnc, args, arg_vars, logging):
  ...
  if ret_type in 'cudaError_t':
    ...
    init_val = '= NULL;'
  elif ret_type in ["cudaChannelFormatDesc", "cublasStatus_t", "cublasStatus", "cusparseStatus_t", \
                    "cusparseMatrixType_t", "cusparseFillMode_t", "cusparseDiagType_t", \
                    "cusparseIndexBase_t", "cusolverStatus_t", "cuComplex", "cuDoubleComplex", \
                    "cufftResult"]:
    init_val = ';'
  ...
  if fnc == "__cudaRegisterFatBinary":
    ...
    unreg= True;
  print("  printf(\"Called at func '%s' in line %i.\\n\", __FUNCTION__, __LINE__ );")
