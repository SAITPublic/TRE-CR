                     ...
                     (MPI_Comm) comm, (MPI_Request *) request)
{
  ...
  if (mana_state == RUNNING &&
    ...
    MPI_Request virtRequest = ADD_NEW_REQUEST(((MPI_Request)(0x2c000000+4)));
#else
...
USER_DEFINED_WRAPPER(int, Open_port, (MPI_Info) info, (char *) port_name)
{
    int retval;
    DMTCP_PLUGIN_DISABLE_CKPT();
    JUMP_TO_LOWER_HALF(lh_info.fsaddr);
    retval = NEXT_FUNC(Open_port)(info, port_name);
    RETURN_TO_UPPER_HALF();
    DMTCP_PLUGIN_ENABLE_CKPT();
    return retval;
}

USER_DEFINED_WRAPPER(int, Close_port, (const char *) port_name)
{
    int retval;
    DMTCP_PLUGIN_DISABLE_CKPT();
    JUMP_TO_LOWER_HALF(lh_info.fsaddr);
    retval = NEXT_FUNC(Close_port)(port_name);
    RETURN_TO_UPPER_HALF();
    DMTCP_PLUGIN_ENABLE_CKPT();
    return retval;
}
...
PMPI_IMPL(int, MPI_Open_port, MPI_Info info, char *port_name)
PMPI_IMPL(int, MPI_Close_port, const char *port_name)
