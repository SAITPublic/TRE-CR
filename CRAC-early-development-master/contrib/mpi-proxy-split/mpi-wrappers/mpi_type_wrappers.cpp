...
USER_DEFINED_WRAPPER(int, Type_create_subarray, (int) ndims, (const int) size_array[],
                     (const int) subsize_array[], (const int) start_array[], (int) order,
                     (MPI_Datatype) oldtype, (MPI_Datatype *) newtype)
{
  int retval;
  DMTCP_PLUGIN_DISABLE_CKPT();
  MPI_Datatype realType = VIRTUAL_TO_REAL_TYPE(oldtype);
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  retval = NEXT_FUNC(Type_create_subarray)(ndims, size_array, subsize_array, start_array, order, realType, newtype);
  RETURN_TO_UPPER_HALF();
  if (retval == MPI_SUCCESS && MPI_LOGGING()) {
    MPI_Datatype virtType = ADD_NEW_TYPE(*newtype);
    *newtype = virtType;
    LOG_CALL(restoreTypes, Type_create_subarray, ndims, size_array, subsize_array, start_array, order, oldtype, virtType);
  }
  DMTCP_PLUGIN_ENABLE_CKPT();
  return retval;
}
...
#if (MANA_USE_OPENMPI)
USER_DEFINED_WRAPPER(MPI_Datatype, Type_f2c, (MPI_Fint) type)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i.\n", __FUNCTION__, __LINE__ );
  MPI_Datatype lhRealType;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  lhRealType = NEXT_FUNC(Type_f2c)(type);
  RETURN_TO_UPPER_HALF();
  MPI_Datatype virtualType = REAL_TO_VIRTUAL_TYPE(lhRealType);
  DMTCP_PLUGIN_ENABLE_CKPT();
  return virtualType;
}

USER_DEFINED_WRAPPER(MPI_Fint, Type_c2f, (MPI_Datatype) type)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i, type = %p.\n", __FUNCTION__, __LINE__ , type);
  MPI_Fint fType;
  DMTCP_PLUGIN_DISABLE_CKPT();
  MPI_Datatype realType = VIRTUAL_TO_REAL_TYPE(type);
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
#if (MANA_USE_OPENMPI)
  GET_LH_HANDLE(realType, (void**)&realType);
#endif
  fType = NEXT_FUNC(Type_c2f)(realType);
  RETURN_TO_UPPER_HALF();
  DMTCP_PLUGIN_ENABLE_CKPT();
  return fType;
}
...
PMPI_IMPL(int, MPI_Type_create_hindexed_block, int count, int blocklength,
          ...
          MPI_Datatype *newtype);
#endif
PMPI_IMPL(int, MPI_Type_create_subarray, int ndims, const int size_array[],
          const int subsize_array[], const int start_array[], int order,
          MPI_Datatype oldtype, MPI_Datatype * newtype);

#if (MANA_USE_OPENMPI)
PMPI_IMPL(MPI_Datatype, MPI_Type_f2c, MPI_Fint type)
PMPI_IMPL(MPI_Fint, MPI_Type_c2f, MPI_Datatype type)
#endif
