...
{
  MPIDLOG(INFO, "Called at func '%s' in line %i, comm = %p.\n", __FUNCTION__, __LINE__, comm);
  int retval;
                     ...
                     (MPI_Comm) comm1, (MPI_Comm) comm2, (int*) result)
{
  ...
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
#if (MANA_USE_OPENMPI)
  GET_LH_HANDLE(realComm1, (void**)&realComm1);
  GET_LH_HANDLE(realComm2, (void**)&realComm2);
#endif
                     ...
                     (int *) comm_keyval, (void *) extra_state)
{
  ...
  int retval = MPI_SUCCESS;
  int keyval = 0;
  for (; keyval < (int)keyvalVec.size(); keyval++) {
...
USER_DEFINED_WRAPPER(int, Comm_free_keyval, (int *) comm_keyval)
{
  ...
  if (keyval < (int)keyvalVec.size() && keyvalVec[keyval] != -1) {
    keyvalVec[keyval] = -1;
...
USER_DEFINED_WRAPPER(int, Comm_accept, (const char *) port_name, (MPI_Info) info,
                     (int) root, (MPI_Comm) comm, (MPI_Comm *) newcomm)
{
    ...
    MPI_Comm realComm = VIRTUAL_TO_REAL_COMM(comm);
    JUMP_TO_LOWER_HALF(lh_info.fsaddr);
    retval = NEXT_FUNC(Comm_accept)(port_name, info, root, realComm, newcomm);
    RETURN_TO_UPPER_HALF();
    if (retval == MPI_SUCCESS && MPI_LOGGING()) {
        MPI_Comm virtComm = ADD_NEW_COMM(*newcomm);
        *newcomm = virtComm;
        LOG_CALL(restoreTypes, Comm_accept, port_name, info, root, comm, virtComm);
    }
    DMTCP_PLUGIN_ENABLE_CKPT();
    return retval;
}

USER_DEFINED_WRAPPER(int, Comm_connect, (const char*) port_name, (MPI_Info) info,
                     (int) root, (MPI_Comm) comm, (MPI_Comm *) newcomm)
{
    int retval;
    DMTCP_PLUGIN_DISABLE_CKPT();
    MPI_Comm realComm = VIRTUAL_TO_REAL_COMM(comm);
    JUMP_TO_LOWER_HALF(lh_info.fsaddr);
    retval = NEXT_FUNC(Comm_connect)(port_name, info, root, realComm, newcomm);
    RETURN_TO_UPPER_HALF();
    if (retval == MPI_SUCCESS && MPI_LOGGING()) {
        MPI_Comm virtComm = ADD_NEW_COMM(*newcomm);
        *newcomm = virtComm;
        LOG_CALL(restoreTypes, Comm_connect, port_name, info, root, comm, virtComm);
    }
    DMTCP_PLUGIN_ENABLE_CKPT();
    return retval;
}
...
#if (MANA_USE_OPENMPI)
USER_DEFINED_WRAPPER(MPI_Comm, Comm_f2c, (MPI_Fint) comm)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i.\n", __FUNCTION__, __LINE__ );
  MPI_Comm lhRealComm;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  lhRealComm = NEXT_FUNC(Comm_f2c)(comm);
  RETURN_TO_UPPER_HALF();
  MPI_Comm virtualComm = REAL_TO_VIRTUAL_COMM(lhRealComm);
  DMTCP_PLUGIN_ENABLE_CKPT();
  return virtualComm;
}

USER_DEFINED_WRAPPER(MPI_Fint, Comm_c2f, (MPI_Comm) comm)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i, comm = %p.\n", __FUNCTION__, __LINE__ );
  MPI_Fint fComm;
  DMTCP_PLUGIN_DISABLE_CKPT();
  ...
  GET_LH_HANDLE(realComm, (void**)&realComm);
#endif
  fComm = NEXT_FUNC(Comm_c2f)(realComm);
  RETURN_TO_UPPER_HALF();
  DMTCP_PLUGIN_ENABLE_CKPT();
  return fComm;
}
...
PMPI_IMPL(int, MPI_Comm_accept, const char *port_name, MPI_Info info, int root, MPI_Comm comm, MPI_Comm *newcomm)
PMPI_IMPL(int, MPI_Comm_connect, const char *port_name, MPI_Info info, int root, MPI_Comm comm, MPI_Comm *newcomm)
#if (MANA_USE_OPENMPI)
PMPI_IMPL(MPI_Comm, MPI_Comm_f2c, MPI_Fint comm)
PMPI_IMPL(MPI_Fint, MPI_Comm_c2f, MPI_Comm comm)
#endif