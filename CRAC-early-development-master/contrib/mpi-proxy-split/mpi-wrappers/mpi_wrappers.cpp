...
#if (MANA_USE_OPENMPI)
  MPI_Comm lhComm;
  GET_LH_HANDLE(MPI_COMM_WORLD, (void**)&lhComm);
  NEXT_FUNC(Comm_dup)(lhComm, &g_world_comm);
#else
...
USER_DEFINED_WRAPPER(int, Initialized, (int *) flag)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i.\n", __FUNCTION__, __LINE__ );
#if (MANA_USE_OPENMPI)
  Initailize_MPIlhinfo;
#endif
  MPIDLOG(INFO, "MPI_Initialized lh_info.fsaddr:%p lh_info.pdlsym:%p pdlsym:%p\n",lh_info.fsaddr, lh_info.lh_dlsym, pdlsym);
  int retval;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
...
USER_DEFINED_WRAPPER(int, Get_version, (int *)version,
                    (int *)subversion)
{
  int retval;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  retval = NEXT_FUNC(Get_version)(version, subversion);
  RETURN_TO_UPPER_HALF();
  DMTCP_PLUGIN_ENABLE_CKPT();
  return retval;
}
...
PMPI_IMPL(int, MPI_Get_version, int *version, int *subversion)
