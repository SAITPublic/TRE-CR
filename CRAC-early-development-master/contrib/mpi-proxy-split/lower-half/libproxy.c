...
getDataFromMaps(const Area *text, Area *heap)
{
  ...
  while (readMapsLineSplit(mapsfd, &area)) {
    //DLOG(INFO,"[XINGBIAO214] area.addr = %p, area.endAddr = %p\n", area.addr, area.endAddr);
...
/**
 * @brief The function of myhandle return the handle value (pointer) of OpenMPI in
 *        lower half of mpi_proxy_split
 *
 * @param handle The enumerated value as the index of array
 * @return void* The handle value in lower half of mpi_proxy_split
 */
void *
myhandle(enum MPI_Handles handle)
{
  // DLOG(NOISE, "handle = %d.\n", handle);
  if (handle < MPI_Handle_Null || handle > MPI_Handle_Invalid) {
    return NULL;
  }

  return MPI_Handle_Ptrs[handle];
}
...
void first_constructor()
{
  ...
  if (firstTime) {
    ...
    lh_info.lh_pdlhandle = (void *)&myhandle;
    ...
    // DLOG(INFO, "lh_memRange.start: %p, lh_memRange.end: %p\n",
    //     lh_memRange.start, lh_memRange.end);
