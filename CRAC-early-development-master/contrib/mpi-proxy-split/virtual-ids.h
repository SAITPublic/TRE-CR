...
#include "dmtcp.h"
#include "mpi-wrappers/mpi_nextfunc.h"
...
#if (MANA_USE_OPENMPI)
#define MpiInfoList  dmtcp_mpi::MpiVirtualization<MPI_Info>
#endif
#ifndef NEXT_FUNC
...
#define REAL_TO_VIRTUAL_COMM_KEYVAL(id) \
  MpiCommKeyvalList::instance("MpiCommKeyval", 0).realToVirtual(id)
#define VIRTUAL_TO_REAL_COMM_KEYVAL(id) \
  MpiCommKeyvalList::instance("MpiCommKeyval", 0).virtualToReal(id)
#define ADD_NEW_COMM_KEYVAL(id) \
  MpiCommKeyvalList::instance("MpiCommKeyval", 0).onCreate(id)
#define REMOVE_OLD_COMM_KEYVAL(id) \
  MpiCommKeyvalList::instance("MpiCommKeyval", 0).onRemove(id)
#define UPDATE_COMM_KEYVAL_MAP(v, r) \
  MpiCommKeyvalList::instance("MpiCommKeyval", 0).updateMapping(v, r)
...
#if (MANA_USE_OPENMPI)
#define REAL_TO_VIRTUAL_INFO(id) \
  MpiInfoList::instance("MpiInfo", MPI_INFO_NULL).realToVirtual(id)
#define VIRTUAL_TO_REAL_INFO(id) \
  MpiInfoList::instance("MpiInfo", MPI_INFO_NULL).virtualToReal(id)
#define ADD_NEW_INFO(id) \
  MpiInfoList::instance("MpiInfo", MPI_INFO_NULL).onCreate(id)
#define REMOVE_OLD_INFO(id) \
  MpiInfoList::instance("MpiInfo", MPI_INFO_NULL).onRemove(id)
#define UPDATE_INFO_MAP(v, r) \
  MpiInfoList::instance("MpiInfo", MPI_INFO_NULL).updateMapping(v, r)
...
#if (MANA_USE_OPENMPI)
  else if (strcmp(name, "MpiInfo") == 0) {
	  static MpiVirtualization _virTableMpiInfo(name, nullId);
	  return _virTableMpiInfo;
	}
#endif
  ...
  class VirtualGlobalCommId {
    public:
      unsigned int createGlobalId(MPI_Comm comm) {
        //comm is the virtual MPI communicator
        // if (comm == MPI_COMM_NULL) {
        //   return (unsigned int)comm;
        // }
        JASSERT(comm != MPI_COMM_NULL)
          .Text("The communicator is null!");
        unsigned int gid = 0;
    ...
    private:
      VirtualGlobalCommId()
      {
        #ifdef MANA_USE_OPENMPI
          MPI_Comm lh_comm_null;
          MPI_Comm lh_comm_world;
          GET_LH_HANDLE(MPI_COMM_NULL,(void **)&lh_comm_null);
          GET_LH_HANDLE(MPI_COMM_WORLD,(void **)&lh_comm_world);
          globalIdTable[lh_comm_null] = (long)MPI_COMM_NULL;
          globalIdTable[lh_comm_world] = (long)MPI_COMM_WORLD;
          globalIdTable[MPI_COMM_NULL] = (long)MPI_COMM_NULL;
          globalIdTable[MPI_COMM_WORLD] = (long)MPI_COMM_WORLD;
        #else
      ...
      void printMap(bool flag = false) {
        for (std::pair<MPI_Comm, unsigned int> idPair : globalIdTable) {
          if (flag) {
            #ifdef MANA_USE_OPENMPI
            printf("virtual comm: %p, real comm: %p, global id: %x\n",
