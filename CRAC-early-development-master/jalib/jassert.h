...
# define JTRACE(msg)                                              \
  if (true) {                                                     \
  } else jassert_internal::JAssert("TRACE", false).JASSERT_CONTEXT("NOTE", \
                                                          msg).JASSERT_CONT_A
#endif // ifdef LOGGING
...
#define JNOTE(msg)          \
  if (jassert_quiet >= 1) { \
    ...
    jassert_internal::JAssert("NOTE", false).JASSERT_CONTEXT("NOTE", msg).JASSERT_CONT_A

#define JWARNING(term)                                                              \
  if ((term) || jassert_quiet >= 2) {                                               \
  } else                                                                            \
    jassert_internal::JAssert("WARNING", false).JASSERT_CONTEXT("WARNING",                     \
...
# define JASSERT(term)              \
  if ((term)) {                     \
    ...
    jassert_internal::JAssert("ASSERT", true) \
