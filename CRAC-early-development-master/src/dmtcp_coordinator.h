...
class LogProfile {
  public:
    void init();
    float pre_process(char* str);
    void generate_ori_data(string api_name, float time);
    float get_interval(std::map<int, std::tuple<string, float, int>>::iterator iter, float interval, float init_interval);
    void profile_log(float init_interval, char* ori_log_path, char* dst_log_path);
};
