...
pthread_mutex_t mutex_for_jnote;
...
dmtcp_initialize_entry_point()
{
  ...
  pthread_mutex_init(&mutex_for_jnote, NULL);
...
int DmtcpWorker::waitForUpdateLogRequest() 
{
  WorkerState::setCurrentState(WorkerState::RUNNING);
  SharedData::resetBarrierInfo();
  ...
  msg.assertValid();
  JTRACE("message type now is...................... :")(msg.type);
  ...
  if (msg.type == DMT_UPDATE_LOG) 
  {
    JTRACE("receive update message success..........");
    return 1;
  } 
  else if (msg.type == DMT_DO_CHECKPOINT) 
  {
    JTRACE("receive do checkpoint msg success...........");
    WorkerState::setCurrentState(WorkerState::PRESUSPEND);
  ...
  else if (msg.type == DMT_USER_ACT) 
  {
    JTRACE("receive user command success..................");
    return 3;
  }
  return -1;
}
...
DmtcpWorker::preCheckpoint()
{
  ...
  // biao.xing@samsung.com added for get local ip 2023.6.2
  struct in_addr saddr;
  dmtcp_get_local_ip_addr(&saddr);
  char *hostip = inet_ntoa(saddr);
  fprintf(stdout, "Process %d checkpoints on %s\n", getpid(), hostip);
...
DmtcpWorker::postRestart(double ckptReadTime)
{
  ...
  dmtcp_get_local_ip_addr(&saddr);
  char *hostip = inet_ntoa(saddr);
  fprintf(stdout, "Process %d restores on %s\n", getpid(), hostip);
