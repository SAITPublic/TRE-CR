...
void mtcp_writememoryareas(int fd, bool bSkipCheck) __attribute__((weak));
/* We specify the ckpt_rank file of the restore */
int FindCkptFile(const dmtcp::string buffer);
int DeleteCkptFile();
static string ckpt_dir;
 ...
 */
static int
test_use_gpu_compression()
{
  char *do_we_compress = NULL;
  do_we_compress = getenv(ENV_VAR_GPU_ALGORITHM);
...
static int
open_ckpt_to_write_gz(int fd, int pipe_fds[2], char *gzip_path, int mode = 1)
{
if(mode == 1)
{
  char *gzip_args[] = {
    ...
    // char tmpZip[1024] = "/usr/bin/time ";
    // strcat(tmpZip, gzip_path);
    // gzip_path = "/usr/bin/time " + gzip_path;
    gzip_args[0] = gzip_path;
    // printf("gzip file path:%s\n", tmpZip);
  ...
  {
    char *gzip_args[] = {
    const_cast<char *>("benchmark_hlif"),
    const_cast<char *>("cascaded"),
    NULL
    };

    gzip_args[0] = "/home/tian01.liu/nvComp/nvcomp/bin/benchmark_hlif";
    gzip_args[1] = getenv(ENV_VAR_GPU_ALGORITHM);

    //  printf("[LT]%s:%d cpu algorithm name:%s\n",__FILE__, __LINE__, gzip_args[1]);

    JTRACE("[LT]open_ckpt_to_write_gz,used cpu compression\n");
                           ...
                           int *fdCkptFileOnDisk)
{
  ...
  *fdCkptFileOnDisk = fd; /* if use_compression, fd will be reset to pipe */
  ...
  use_gzip_compression = test_use_compression(const_cast<char *>("GZIP"),
                                              gzip_cmd, gzip_path, 0);
  // if not gzip,then judge whether gpu compress is selected.
  if (!use_gzip_compression)
  {
    use_gzip_compression = test_use_gpu_compression();
    JTRACE("use gpu compression\n")(use_gzip_compression);
  }
  ...
  */
  ...
  if (use_deltacompression || use_gzip_compression) { /* fork compr. process */
    ...
    if (use_deltacompression) { /* fork a hbict process */
      ...
      *use_compression = true;
      fd = open_ckpt_to_write_gz(fd, pipe_fds, gzip_path, use_gzip_compression);
      if (pipe_fds[0] == -1) {
...
int FindCkptFile(const dmtcp::string buffer)
{
  ...
  vector<string> files = jalib::Filesystem::ListDirEntries(buffer);
  for (const string &file : files) 
  {
      if (Util::strStartsWith(file.c_str(), "ckpt") && Util::strEndsWith(file.c_str(), ".dmtcp")) 
      {
        dmtcp::ostringstream o;
        o << buffer << "/";
        dmtcp::string tmpdir  = o.str();
        ckpt_dir = tmpdir + file;
        int result = remove((tmpdir + file).c_str());
        JASSERT(result == 0)
        .Text("Failed to remove ckpt_rank file.");
        break;
      }
  }
  return 0;
...
int DeleteCkptFile()
{
  if (fopen(ckpt_dir.c_str(), "r") == NULL)
  {
    return 1;
  }
  int result = remove(ckpt_dir.c_str());
  JASSERT(result == 0)
  .Text("Failed to remove ckpt_rank file.");
  return 0;
}
...
CkptSerializer::writeCkptImage(void *mtcpHdr, size_t mtcpHdrLen)
{
  ...
  FindCkptFile(ProcessInfo::instance().getCkptDir());
  ...
  bool bSkipCheck = strstr(tempCkptFilename.c_str(), "ckpt_kernel-loader.exe");
  mtcp_writememoryareas(fd, bSkipCheck);
