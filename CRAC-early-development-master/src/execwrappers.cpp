...
static vector<string>
patchUserEnv(vector<string>env, const char *filename, int appendPluginLibs)
{
  vector<string>result;
...
execve(const char *filename, char *const argv[], char *const envp[])
{
  ...
  char *newFilename;
  char **newArgv;
  const char *kernelLoader = getenv(ENV_VAR_KERNEL_LOADER);
  dmtcpPrepareForExec(filename, argv, &newFilename, &newArgv);
  vector<string>envStrings;

  vector<string>argv_ex = copyEnv(newArgv);
  if (strstr(filename, "kernel-loader") == NULL)
  {
    newFilename = const_cast<char*>(kernelLoader);
    argv_ex.insert(argv_ex.begin(), string(kernelLoader));
    argv_ex[1] = string(filename);
  }

  string preloadLibs = getUpdatedLdPreload(filename, NULL);
  if (kernelLoader != NULL && strcmp(newFilename, kernelLoader) == 0) {
    // KernelLoader, append cuda plugin
    envStrings = patchUserEnv(env, filename, 0);
    preloadLibs += ":";
    preloadLibs += getenv(ENV_VAR_PLUGIN);
    string uhPreloadStr = "UH_PRELOAD=";
    uhPreloadStr += preloadLibs;
    envStrings.push_back(uhPreloadStr);
  } else {
    envStrings = patchUserEnv(env, filename, 1);
  }

  if (strstr(filename, "kernel-loader") == NULL)
  {
    vector<string>::iterator it_del = envStrings.end();
    for (int i = 0; i < envStrings.size(); i++)
    {
      if (strstr(envStrings[i].c_str(), "LD_PRELOAD"))
      {
        envStrings[i][11] = '\0';
      }
    }
  ...
  const vector<const char *>newArgv_ex = stringVectorToPointerArray(argv_ex);
  personality(ADDR_NO_RANDOMIZE);
  // int retVal = _real_execve(newFilename, newArgv, (char *const *)&newEnv[0]);
  int retVal = _real_execve(newFilename, (char *const *)&newArgv_ex[0], (char *const *)&newEnv[0]);
...
execvpe(const char *filename, char *const argv[], char *const envp[])
{
  ...
  const char *kernelLoader = getenv(ENV_VAR_KERNEL_LOADER);
  dmtcpPrepareForExec(filename, argv, &newFilename, &newArgv);
  vector<string>newEnvStrings;

  string preloadLibs = getUpdatedLdPreload(filename, NULL);
  if (kernelLoader != NULL && strcmp(newFilename, kernelLoader) == 0) {
    // KernelLoader, append cuda plugin
    newEnvStrings = patchUserEnv(env, filename, 0);
    preloadLibs += ":";
    preloadLibs += getenv(ENV_VAR_PLUGIN);
    string uhPreloadStr = "UH_PRELOAD=";
    uhPreloadStr += preloadLibs;
    newEnvStrings.push_back(uhPreloadStr);
  } else {
    newEnvStrings = patchUserEnv(env, filename, 1);
  }
