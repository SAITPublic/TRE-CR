  ...
  while (procSelfMaps.getNextArea(&area)) {
    if ((area.flags & MAP_SHARED) && area.prot != 0) {
      ...
      if (Util::isNscdArea(area) ||
          ...
          || Util::isNCCLShmArea(area)
#endif
          ) // by tian01.liu add for nccl shm 2023.9.4
      {
        continue;
      }
      ...
      //biao.xing@samsung.com added 2023.4.8
      ...
      if (jalib::Filesystem::FileExists(area.name)) {
        ...
        if (Util::strStartsWith(area.name, DEV_ZERO_DELETED_STR) ||
          ...
          JNOTE("Will recreate shm file on restart.") (area.name);
...
FileConnList::recreateShmFileAndMap(const ProcMapsArea &area)
{
  ...
  JNOTE("recreateShmFileAndMap....")(area.name);
...
FileConnList::scanForPreExisting()
{
  ...
  for (size_t i = 0; i < fds.size(); ++i) {
    ...
    if (dmtcp_is_bq_file && dmtcp_is_bq_file(device.c_str())) {
       ...
       */
      continue;
    } else if (getenv("B2_JOBID") && Util::strEndsWith(device.c_str(), ".log")) {
      /*
       * The output file named ID.log(1000101.log) is generated by Samsung S2 when checkpointing,
       * however, a new log file(1000102.log) is generated when restoring. 1000101.log is useless
       * when restoring, so we should not record it. Now, skip to add file connection when lanunch
       * TER-CR by S2 and the file is ended with ".log".
       */
      continue;
                                    ...
                                    mode_t mode)
{
  ...
  JTRACE("st_mode")(path)(statbuf.st_mode);
  path = device.c_str();
