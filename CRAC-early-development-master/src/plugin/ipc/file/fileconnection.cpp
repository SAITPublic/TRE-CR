...
_isOpenMPIApp()
{
  static int isOpenMpiApp = -1;
  if (isOpenMpiApp == -1) {
    string progName = jalib::Filesystem::GetProgramName();

    if (progName == "orterun" || progName == "orted") {
      isOpenMpiApp = 1;
    } else {
      isOpenMpiApp = 0;
    }
...
FileConnection::drain()
{
  ...
  if (dmtcp_should_ckpt_open_files() && statbuf.st_uid == getuid()) {
    ...
    _ckpted_file = true;
#endif // if 0
  } else if (/*_isOpenMPIApp() && */Util::strEndsWith(_path.c_str(), ".sm")) {
    JNOTE("handle openmpi sm file")(_path.c_str());
    _ckpted_file = true;
  } else {
    _ckpted_file = false;
  }
