...
shmget(key_t key, size_t size, int shmflg)
{
  ...
  if (realKey == -1) {
    if (key != IPC_PRIVATE) {
      ...
      /*
       * The description of shmget: A new shared memory segment, with size equal to the value of
       * size rounded up to a multiple of PAGE_SIZE, is created if key has the value IPC_PRIVATE
       * or key isn't IPC_PRIVATE, no shared memory segment corresponding to key exists, and
       * IPC_CREAT is specified in shmflg. Set the key to realKey, when the key is IPC_PRIVATE,
       */
      realKey = key;
    }
  }
