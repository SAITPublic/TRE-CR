...
ProcessInfo::init()
{
  ...
  //If current process is kernel-loader, please not invoke growStack()
  if (strcmp(programName.c_str(), "kernel-loader.exe") != 0)
    growStack();
...
ProcessInfo::restart()
{
  ...
  // printf("processInfo restart .........._ckptFileName = %s\n", _ckptFileName.c_str());
  if (strstr(_ckptFileName.c_str(), "ckpt_kernel-loader.exe") == 0)
  {
    restoreHeap();
  }
