...
Util::getDmtcpArgs(void)
{
  ...
  const char *targetLd = getenv(ENV_VAR_TARGET_LD);
  ...
  if (kernelLoader != NULL) {
    argVector.push_back("--kernel-loader");
    argVector.push_back(kernelLoader);
  }

  if (targetLd != NULL) {
    argVector.push_back("--target-ld");
    argVector.push_back(targetLd);
  }
