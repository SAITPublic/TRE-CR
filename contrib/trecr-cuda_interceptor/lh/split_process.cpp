...
// #include "../mpi-proxy-split/switch_context.h"
// #include "../mpi-proxy-split/lower-half/procmapsutils.h"
// #include "../../src/constants.h"
#include "lower_half_api.h"
#include "../../trecr-mpi_interceptor/switch_context.h"
#include "split_process.h"
#include "../utils/mtcp_util.h"
#include "procmapsutils.h"
// #include "restore_mem/mtcp_sys.h"
...
EXTERNC int readMapsLineSplit(int , Area* );
static int writeMPILhInfoToFile();
...
read_lh_proxy_bits(pid_t childpid)
{
  ...
  // get the metadata and map the memory segment
    remote_iov[i].iov_base = split_lh_regions_list[i].start_addr;
    remote_iov[i].iov_len = (unsigned long)split_lh_regions_list[i].end_addr -
                            (unsigned long)split_lh_regions_list[i].start_addr;
    ret = mmap_iov(&remote_iov[i], split_lh_regions_list[i].prot | PROT_WRITE);
...
startProxy()
{
  ...
  switch (childpid) {
    ...
    default: // in parent
    {
      ...
      JWARNING(dmtcp::Util::readAll(pipefd_out[0], &split_lh_regions_list, total_bytes)
...
findLHMemRange(MemRange_t *lh_mem_range)
{
  ...
  if (readMapsLineSplit(mapsfd, &area)) {
  ...
  while (readMapsLineSplit(mapsfd, &area)) {
    ...
    if ((prev_addr_end + 2 * ONEGB + PAGE_SIZE) <= next_addr_start) {
      ...
      // fprintf(stderr, "*** [XINGBIAO] find next_addr_start = %p, start = %p, end = %p\n",
      //  next_addr_start, lh_mem_range->start, lh_mem_range->end);
      is_set = true;
      break;
    }
...
initializeLowerHalf()
{
  ...
  syscall(SYS_arch_prctl, ARCH_SET_FS, lh_info.fsaddr);
...
// Writes out the MPIlhinfo global object to a file. Returns 0 on success,
// -1 on failure.
static int
writeMPILhInfoToFile()
{
  size_t rc = 0;
  char filename[100];
  snprintf(filename, 100, "./mpiLhInfo_%d", getpid());
