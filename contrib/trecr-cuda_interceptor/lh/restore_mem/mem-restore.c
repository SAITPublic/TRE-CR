...
extern void readUhInfoAddr();
extern void* g_stack_seg_addr;
extern size_t g_stack_seg_size;
...
{
  DLOG(INFO, "Restore checkpoint image ...\n");
  assert(ckptfd != -1);
             ...
             MYINFO_GS_T myinfo_gs)
{
  ...
  *(pid_t *)(*(unsigned long *)&(tlsInfo->gdtentrytls[0].base_addr)
  ...
  //syscall(SYS_arch_prctl, ARCH_SET_FS, *(unsigned long *)&(mtcpHdr.motherofall_tls_info.gdtentrytls[0].base_addr));
...
readMtcpHeader(int ckptFd)
{
  ...
  DLOG(INFO, "mtcpHdr.orig_pid =%d, signature = %s", mtcpHdr.orig_pid, mtcpHdr.signature);
  DLOG(INFO, "base_addr = %p\n", (pid_t *)(*(unsigned long *)&((&mtcpHdr.motherofall_tls_info)->gdtentrytls[0].base_addr)));
  return 1;
}
...
restoreMemoryRegion(int ckptfd, Area* area)
{
  ...
  ssize_t bytes = 0;
  void * mmappedat;
  if(area->addr == g_stack_seg_addr)
  {
    g_stack_seg_size = area->size;
  }
