...
splitProcess(RestoreInfo *rinfo)
{
  ...
  compileProxy();
#endif
  // MTCP_PRINTF("Initializing Lower-Half Proxy\n");
  ...
  if (childpid > 0) {
    ...
    // MTCP_PRINTF("kill lh_proxy process...\n");
  }
  if (ret == 0) {
...
startProxy(RestoreInfo *rinfo)
{
  ...
  switch (childpid) {
    ...
    default: // in parent
    {
      ...
      // MTCP_PRINTF("*** UH -> LH, start: %p, end: %p\n", mem_range.start, mem_range.end);
...
setLhMemRange(RestoreInfo *rinfo)
{
  ...
  if (found) {
#if 0//!defined(MANA_USE_LH_FIXED_ADDRESS)
    lh_mem_range.start = (VA)area.addr - TWO_GB;
    lh_mem_range.end = (VA)area.addr - ONE_GB;
#else
    //The address of restoration is between [heap] and lower half
