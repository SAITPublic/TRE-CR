...
{
  //printf("UH, addr = %p, length = 0x%zx, prot = %d, flags = %d, fd = %d, offset = 0x%zx\n", addr, length, prot, flags, fd, offset);
...
{
  printf("[lt=============enter realloc in crac========]\n");
  static __typeof__(&realloc) lowerHalfReallocWrapper = (__typeof__(&realloc)) - 1;
  if (!initialized) {
    initialize_wrappers();
  }
  if (lowerHalfReallocWrapper == (__typeof__(&realloc)) - 1) {
    lowerHalfReallocWrapper = (__typeof__(&realloc))lhInfo.lhRealloc;
  }
  // TODO: Switch fs context
  return lowerHalfReallocWrapper(addr, size);
}*/
