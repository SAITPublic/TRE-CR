...
namespace dmtcp_mpi
{
    ...
    struct FncArg
    {
        ...
        {
            return *(MPI_Info*)_data;
        }

        operator MPI_Errhandler() const
        {
            return *(MPI_Errhandler*)_data;
        }
    ...
    FncArg FncArgTyped(T data)
    {
        ...
        if (typeid(data) == typeid(h))
        {
            return FncArg(&data, sizeof(data), TYPE_INT_CONST_PTR);
        }
#ifdef MANA_USE_OPENMPI
        if (typeid(data) == typeid(aint))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_AINT);
        }
        if (typeid(data) == typeid(offset))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_OFFSET);
        }
        if (typeid(data) == typeid(count))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_COUNT);
        } 
        if (typeid(data) == typeid(comm))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_COMM);
        }
        if (typeid(data) == typeid(datatype))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_DATATYPE);
        }
        if (typeid(data) == typeid(errhandler))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_ERRHANDLER);
        }
        if (typeid(data) == typeid(file))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_FILE);
        }
        if (typeid(data) == typeid(group))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_GROUP);
        }
        if (typeid(data) == typeid(info))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_INFO);
        }
        if (typeid(data) == typeid(op))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_OP);
        }
        if (typeid(data) == typeid(quest))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_REQUEST);
        } 
        if (typeid(data) == typeid(message))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_MESSAGE);
        }
        if (typeid(data) == typeid(status))
        {
            return FncArg(&data, sizeof(data), TYPE_MPI_STATUS);
        } 
#endif
    ...
    class MpiRecord
    {
        ...
        {
            // std::cout << "name" << typeid(arg).name() << "," << typeid(MPI_Comm).name() <<std::endl;
