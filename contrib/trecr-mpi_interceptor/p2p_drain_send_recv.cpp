...
registerLocalSendsAndRecvs()
{
  ...
  //fprintf(stderr, "*** g_bytesSentToUsByRank[%d] = %d\n", g_world_rank, g_bytesSentToUsByRank[g_world_rank]);
...
localRankToGlobalRank(int localRank, MPI_Comm localComm)
{
  ...
  NEXT_FUNC(Comm_group)(lhComm, &worldGroup);
#else
