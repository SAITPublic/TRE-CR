...
#include "../trecr-cuda_interceptor/lh/split_process.h"
...
#define lh_regions_list split_lh_regions_list
...
#undef dmtcp_skip_memory_region_ckpting_mpi
...
void recordMpiInitMaps()
{
  ...
  if (mpiInitLhAreas->size() > 0)
  {
    ...
    //biao.xing: Mapped by MPI_Init/MPI_Init_thread
  ...
  if (lh_info.numCoreRegions > 0) {
    ...
    //biao.xing: lower half's text segment, data segment and heap 
  ...
  if (g_list) {
    ...
    //biao.xing: lower half's mmapped memory region
...
void recordOpenFds()
{
  ...
  for (int fd : fds) {
    ...
    //devices are pipe:[288358632], socket:[288330395]
    o << fd << " -> " << device << "\n";
  }
...
isLhDevice(const ProcMapsArea *area)
{
  if (strstr(area->name, "/dev/zero") ||
      ...
      // strstr(area->name, "/dev/shm") note by tian01.liu for nccl 2023.8.14
      ) {
    return true;
  }
...
EXTERNC int
dmtcp_skip_memory_region_ckpting_mpi(const ProcMapsArea *area)
{
  if (isLhDevice(area)) {
...
getLhMmapList()
{
  ...
  //the memory regions of mmapping are stored in g_list
  JTRACE("Lower half region info")(g_numMmaps);
  for (int i = 0; i < g_numMmaps; i++) {
    JTRACE("Lh region")(g_list[i].addr)(g_list[i].len)(g_list[i].unmapped);
    //fprintf(stderr, "[LH REGION], addr = %p, len = %d, unmapped = %d\n", g_list[i].addr, g_list[i].len, g_list[i].unmapped);
  }
}
...
mpi_plugin_event_hook(DmtcpEvent_t event, DmtcpEventData_t *data)
{
  switch (event) {
    case DMTCP_EVENT_INIT: {
      ...
      initialize_signal_handlers();
      initialize_segv_handler();
      // splitProcess will be called in contrib/split-cuda/kernel-loader.cpp 
