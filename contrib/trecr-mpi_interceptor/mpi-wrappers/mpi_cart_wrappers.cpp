                     ...
                     (const int*) remain_dims, (MPI_Comm *) new_comm)
{
  ...
  DMTCP_PLUGIN_DISABLE_CKPT();
  MPI_Comm realComm = VIRTUAL_TO_REAL_COMM(comm);
  MPIDLOG(INFO, "Called at func '%s' in line %i comm:%p realComm:%p MPI_COMM_WORLD:%p remain_dims:%d . 111\n", __FUNCTION__, __LINE__, comm, realComm, MPI_COMM_WORLD, *remain_dims);
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
#if (MANA_USE_OPENMPI)
  ...
  RETURN_TO_UPPER_HALF();
  MPIDLOG(INFO, "Called at func '%s' in line %i comm:%p realComm:%p MPI_COMM_WORLD:%p retval:%d. 222\n", __FUNCTION__, __LINE__, comm, realComm, MPI_COMM_WORLD, retval);
  if (retval == MPI_SUCCESS && MPI_LOGGING()) {
    int ndims = 0;
    MPI_Cartdim_get(comm, &ndims);
    MPIDLOG(INFO, "Called at func '%s' in line %i ndims:%d.\n", __FUNCTION__, __LINE__ , ndims);
                     ...
                     (MPI_Comm *) comm_cart)
{
  ...
  RETURN_TO_UPPER_HALF();
  MPIDLOG(INFO, "old_comm:%p realComm:%p MPI_COMM_WORLD:%p *comm_cart:%p\n", old_comm, realComm, MPI_COMM_WORLD, *comm_cart);
  if (retval == MPI_SUCCESS && MPI_LOGGING()) {
    MPI_Comm virtComm = ADD_NEW_COMM(*comm_cart);
    VirtualGlobalCommId::instance().createGlobalId(virtComm);
    *comm_cart = virtComm;
