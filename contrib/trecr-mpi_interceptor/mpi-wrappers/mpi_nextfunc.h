...
#include "convert.h"
#define Initailize_MPIlhinfo MPIinitialize_wrappers()
extern void MPIinitialize_wrappers();
extern int MPIinitialized;
...
#ifndef GET_LH_HANDLE
#define GET_LH_HANDLE(uh_handle, lh_handle_ptr)                                \
  ({                                                                           \
    MPI_Handles handle_id = getMpiType(uh_handle);                             \
    if (MPI_Handle_Null != handle_id && MPI_Handle_Invalid != handle_id) {     \
      *lh_handle_ptr = pdlhandle(handle_id);                                   \
    } else {                                                                   \
      *lh_handle_ptr = uh_handle;                                              \
    }                                                                          \
  })
#endif // ifndef GET_LH_HANDLE
