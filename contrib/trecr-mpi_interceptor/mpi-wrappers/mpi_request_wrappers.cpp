...
#if (MANA_USE_OPENMPI)
USER_DEFINED_WRAPPER(MPI_Request, Request_f2c, (MPI_Fint) request)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i.\n", __FUNCTION__, __LINE__ );
  MPI_Request lhRealRequest;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  lhRealRequest = NEXT_FUNC(Request_f2c)(request);
  RETURN_TO_UPPER_HALF();
  MPI_Request virtualRequest= REAL_TO_VIRTUAL_REQUEST(lhRealRequest);
  DMTCP_PLUGIN_ENABLE_CKPT();
  return virtualRequest;
}

USER_DEFINED_WRAPPER(MPI_Fint, Request_c2f, (MPI_Request) Request)
{
  ...
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
#if (MANA_USE_OPENMPI)
  GET_LH_HANDLE(realRequest, (void**)&realRequest);
#endif
  fRequest = NEXT_FUNC(Request_c2f)(realRequest);
  RETURN_TO_UPPER_HALF();
  DMTCP_PLUGIN_ENABLE_CKPT();
  return fRequest;
}

USER_DEFINED_WRAPPER(int, Status_f2c, (const MPI_Fint) *f_status, (MPI_Status *) c_status)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i.\n", __FUNCTION__, __LINE__ );
  int retval;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  retval = NEXT_FUNC(Status_f2c)(f_status,c_status);
  RETURN_TO_UPPER_HALF();
  DMTCP_PLUGIN_ENABLE_CKPT();
  return retval;
}

USER_DEFINED_WRAPPER(int, Status_c2f, (const MPI_Status *) c_status, (MPI_Fint *) f_status)
{
  MPIDLOG(INFO, "Called at func '%s' in line %i, comm = %p.\n", __FUNCTION__, __LINE__ );
  int retval;
  DMTCP_PLUGIN_DISABLE_CKPT();
  JUMP_TO_LOWER_HALF(lh_info.fsaddr);
  retval = NEXT_FUNC(Status_c2f)(c_status, f_status);
  RETURN_TO_UPPER_HALF();
  DMTCP_PLUGIN_ENABLE_CKPT();
  return retval;
}
...
PMPI_IMPL(int, MPI_Request_get_status, MPI_Request request, int* flag,
          MPI_Status *status)
#if (MANA_USE_OPENMPI)
PMPI_IMPL(MPI_Request, MPI_Request_f2c, MPI_Fint request)
PMPI_IMPL(MPI_Fint, MPI_Request_c2f, MPI_Request request)
PMPI_IMPL(int, MPI_Status_f2c, const MPI_Fint *f_status, MPI_Status *c_status)
PMPI_IMPL(int, MPI_Status_c2f, const MPI_Status * c_status, MPI_Fint * f_status)
#endif
