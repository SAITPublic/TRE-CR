...
#define GENERATE_HANDLE_ENUM(HANDLE)    MPI_Handle_##HANDLE
#define GENERATE_HANDLE(HANDLE)  MPI_##HANDLE
#define GENERATE_HANDLE_PTR(HANDLE)  &MPI_##HANDLE
//#define GENERATE_HANDLE_STRING(HANDLE)  "MPI_" #HANDLE
 ...
 *        which is in "contrib/split-cuda/restart_plugin/restore_info.h"
 *
 */
...
enum MPI_Handles {
  MPI_Handle_Null,
  FOREACH_HANDLE(GENERATE_HANDLE_ENUM)
  MPI_Handle_Invalid,
};
...
typedef void* (*proxyHandle_t)(enum MPI_Handles handle);
...
extern MPILowerHalfInfo_t MPIlh_info;
...
extern proxyHandle_t pdlhandle; //Get the lh_handle according to the uh_handle
...
extern void *myhandle(enum MPI_Handles handle);
