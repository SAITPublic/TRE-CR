...
static const char *singleHostProcessing =
  ...
  "exec $dmt_rstr_cmd $coordinator_info --cuda $ckpt_dir \\\n"
...
static const char *multiHostProcessing =
  ...
  "      $new_ckpt_files_group\n"
  "  else\n"
  "    if [ \"$worker_host\" == \"gsp_15\" ]; then\n"
  "      $dmt_rstr_cmd --coord-host $coord_host"
                                                " --coord-port $coord_port\\\n"
  "      --cuda $ckpt_dir --join-coordinator --interval \"$checkpoint_interval\""
                                                " $tmpdir \\\n"
  "      $new_ckpt_files_group\n"
  "    else\n"
  "      if [ -z $ncclCommId ]; then\n"
  "        ncclCommId = 127.0.0.1:78900\n"
  "      else\n"
  "        echo \"user set ncclCommId\"\n"
  "      fi\n"
  ...
  "        \"/bin/sh -c \'cd $DMTCP_RESTART_DIR;NCCL_COMM_ID=$ncclCommId $dmt_rstr_cmd --coord-host $coord_host"
                                                " --coord-port $coord_port\\\n"
  "        --cuda $ckpt_dir --join-coordinator --interval \"$checkpoint_interval\""
                                                " $tmpdir \\\n"
  "        $new_ckpt_files_group\'\" &\n"
