...
syscall(long sys_num, ...)
{
  ...
  /* begin: by tian01.liu, for intecept the syscall in ompi patcher-overwrite */
  case SYS_mmap:
  {
    SYSCALL_GET_ARGS_6(void*, addr, size_t, length, int, prot, int, flags, int, fd, off_t, offset);
    ret = (unsigned long)mmap(addr, length, prot, flags, fd, offset);
    break;
  }
  case SYS_munmap:
  {
  ...
  /* end  : by tian01.liu, for intecept the syscall in ompi patcher-overwrite */
  case SYS_getpid:
  {
    ret = getpid();
    break;
