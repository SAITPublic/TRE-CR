 ...
 *****************************************************************************/
void
mtcp_writememoryareas(int fd, bool bSkipCheck)
{
  Area area;
  ...
  while (procSelfMaps->getNextArea(&area)) {
    ...
    if ((uint64_t)area.addr == ProcessInfo::instance().restoreBufAddr()) {
      ...
      if (area.size == ProcessInfo::instance().restoreBufLen())
          continue;
    ...
    if (area.size == 0) {
      ...
      // fprintf(stderr, "skipping over zero-sized segment, area.addr = %p, area.size = %d\n",
      //   (void*)area.addr, area.size);
      continue;
    }
    ...
    if (getenv(ENV_VAR_MPI_SUPPORTED) != nullptr &&
        ...
        if (dmtcp_skip_memory_region_ckpting_mpi &&
            dmtcp_skip_memory_region_ckpting_mpi(&area)) {
          JTRACE("skipping over memory section as suggested by plugin")
            (area.name) ((void*)area.addr) (area.size);
          // fprintf(stderr, "skipping memory, area.name = %s, area.addr = %p, area.size = %d\n",
          //   area.name, (void*)area.addr, area.size);
          continue;
        }
    }
    ...
    if (0 == strcmp(area.name, "[vsyscall]") ||
      ...
      JNOTE("skipping vDSO special section")
        (area.name) ((void*)area.addr) (area.size);
      continue;
    ...
    JTRACE("before calling to skip") ((void *)area.addr) ((void *)area.endAddr) (area.size);
    if (bSkipCheck != 0) {
      int skip = dmtcp_skip_memory_region_ckpting_ex(&area, fd, stack_was_seen);
      JTRACE("after calling to skip ex") ((void *)area.addr) ((void *)area.endAddr) (area.size) (skip);

      // added by tian01.liu for bug of realloc, 2023.1.20
      if (skip) {
          skip = dmtcp_skip_memory_region_ckpting(&area, fd, stack_was_seen);
          JTRACE("after calling to skip") ((void *)area.addr) ((void *)area.endAddr) (area.size) (skip);
      }

      if (skip != 0) {
         JTRACE("Skipping region as requested by the plugin") ((void *)area.addr)
