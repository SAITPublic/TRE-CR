...
#define WRAPPER_EXECUTION_DISABLE_CKPT()           \
  /*JTRACE("Acquiring wrapperExecutionLock...WRAPPER_EXECUTION_DISABLE_CKPT");*/    \
  bool __wrapperExecutionLockAcquired =            \
    dmtcp::ThreadSync::wrapperExecutionLockLock(); \
  if (__wrapperExecutionLockAcquired) {            \
    /*JTRACE("Acquired wrapperExecutionLock...WRAPPER_EXECUTION_DISABLE_CKPT");*/   \
  }

#define WRAPPER_EXECUTION_ENABLE_CKPT()              \
  if (__wrapperExecutionLockAcquired) {              \
    /*JTRACE("Releasing wrapperExecutionLock...WRAPPER_EXECUTION_ENABLE_CKPT");*/    \
...
void setFlag(char* name, int count);
...
class WrapperLock
{
  public:
      ...
      : exclusiveLock(_exclusiveLock)
    {
      ...
      if (exclusiveLock) {
        JNOTE("WrapperLock...for exclusiveLock..0");
        lockAcquired = ThreadSync::wrapperExecutionLockLockExcl();
        dmtcp::ThreadSync::unsetOkToGrabLock();
      } else {
        JNOTE("WrapperLock...for exclusiveLock..1");
