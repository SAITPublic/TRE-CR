...
static const char *theUsage =
  ...
  "              Disable all plugins.\n"
  "  --cuda\n"
  "              Enable GPU checkpointing.\n"
  "  --mpi \n"
  "              Enable MPI checkpointing.\n"
  "\n"
  "Other options:\n"
  ...
  "  --kernel-loader\n"
  "              Set the path of kernel-loader.exe via updating environment variable named ENV_VAR_KERNEL_LOADER\n"
  "              (this configuration is invalid without setting '--cuda')\n"
  "  --target-ld\n"
  "              Set the path of glibc/lib/ld via updating environment variable named ENV_VAR_TARGET_LD"
  "  --help\n"
...
static bool enableMpiKernelLoader = false;
...
processArgs(int *orig_argc, char ***orig_argv)
{
  ...
  while (true) {
    ...
    if ((s == "--help") && argc <= 1) {
      ...
      shift;
    } else if (s == "--gpu-compress") {
      /*set checkpoint img file used gpu to compress by tian01.liu*/
      setenv(ENV_VAR_GPU_ALGORITHM, argv[1], 1);
      shift; shift;
    }
#ifdef HBICT_DELTACOMP
    else if (s == "--hbict") {
      ...
      /* exec mpi + kernel loader with the environment variables*/
      enableMpiKernelLoader = true;
      shift;
    } else if (s == "--mpi") {
...
main(int argc, char **argv)
{
  ...
  if (testScreen(argv, &newArgv)) {
    ...
    for (int i = 0; i < argc; i++) {
      // lu77.wei added
      if (i == 5)  newArgs.push_back(kernelLoaderPath);
...
testJava(char **argv)
{
  ...
  if ((fd = open("/proc/meminfo", O_RDONLY)) != -1 &&
      ...
      atol(buf + sizeof(meminfoPrefix)) > 17000000) { // units of kB : mem > 4GB
...
setLDPreloadLibs(bool is32bitElf)
{
  ...
  if (enableKernelLoader && getenv(ENV_VAR_PLUGIN) != NULL) {
