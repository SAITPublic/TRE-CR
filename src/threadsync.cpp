...
extern __thread int is_checkpoint_thread;
...
ThreadSync::acquireLocks()
{
  ...
  pthread_rwlock_wrlock(&thread_lock); // leeyy_Note[epic=multithreadlock,seq=1283] pthread_rwlock_wrlock
...
// support timing by huiru.deng
void ThreadSync::setFlag(char* name, int count) {
  strcpy(name_api, name);
  g_count = count;
}
...
extern bool g_b_suspend;
__thread bool is_real_api_exe;
__thread bool g_mutext_first = true;
__thread pthread_mutex_t thread_mutex;
__thread size_t mutex_lock_size = 0;
__thread bool is_new = false;
...
dmtcp_plugin_disable_ckpt()
{
  ...
  if (pthread_rwlock_rdlock(&thread_lock) == 0)
  {
    return 1;
  }
  return 0;
...
extern "C"
int
dmtcp_get_count() // support timing by huiru.deng
{
  return g_count;
}
...
extern "C"
char*
dmtcp_get_name() // support timing by huiru.deng
{
  return name_api;
}
...
extern "C"
int
dmtcp_plugin_disable_ckptthread()
{
  // printf("in dmtcp_plugin_disable_ckptthread\n");
  return DmtcpMutexLock(&theCkptCanStart);
}
...
{
  // printf("in dmtcp_plugin_enable_ckptthread\n");
  DmtcpMutexUnlock(&theCkptCanStart);
}
