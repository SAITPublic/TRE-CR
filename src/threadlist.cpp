...
extern pthread_mutex_t mutex_for_jnote;
...
// support timing by huiru.deng
extern "C"
void doCheckpoint(int i)
{
  DmtcpMessage msg(DMT_DO_CHECKPOINT2);
...
// support timing by huiru.deng
/*----------------------------------------Start of huiru.deng function define----------------------------------*/
...
void parseCount(std::map<std::string, int> &api_count)
{
  api_count.clear();
  FILE* fp_2 = NULL;
  fp_2 = fopen(dst_log_path_1, "r");
  char name_api[100];
  int total_num;

  while (fscanf(fp_2, "%s %d", &name_api, &total_num) != EOF)
  {
    std::string tem_name = name_api;
    api_count.insert({tem_name, total_num});
  }
  fclose(fp_2);
}

void initMap(std::map<int, std::pair<std::string, int>> &checkpoint_location)
{
  // parse checkpoint location info
  checkpoint_location.clear();
  FILE* fp_1 = NULL;
  fp_1 = fopen(dst_log_path, "r");
  char name_checkpoint[100];
  int count;
  int order;

  while (fscanf(fp_1, "%d %s %d", &order, &name_checkpoint, &count) != EOF)
  {
    std::string tem_name_check = name_checkpoint;
    checkpoint_location.insert({order, {tem_name_check, count}});
  }

  fclose(fp_1);
}

void parseLastApi(std::string& last_api_name)
{
  FILE* fp_3 = NULL;
  fp_3 = fopen(dst_log_path_2, "r");
  char name[100];
  fscanf(fp_3, "%s\n", &name);
  last_api_name = name;
  fclose(fp_3);
}
/* ----------------------------------------End of huiru.deng function define-------------------------------*/
__thread int is_checkpoint_thread = 0;
static void *
checkpointhread(void *dummy)
{
  ...
  executeThread();
#else
  JNOTE("Disable timing execution in checkpoint thread !");
#endif
...
#define SIGCANCEL (__SIGRTMIN) /* aka SIGTIMER */
  ...
  JNOTE("after sigsetjmp/getcontext")
  ...
  if (originalstartup) {
     ...
     */
    JNOTE("waiting for other threads after restore");
    ThreadList::waitForAllRestored(ckptThread);
    JNOTE("resuming after restore");
  }
...
#ifdef TIMING
  std::map<int, std::pair<std::string, int>> checkpoint_location;
  std::map<std::string, int> api_count;
  std::map<int, std::pair<std::string, int>>::iterator iter;
  std::string last_API_name;
  int flag = -1;
  int cycle = 0;

  parseCount(api_count);
  parseLastApi(last_API_name);
#else
  printf("Disable timing execution in parse info !\n");
#endif

while (1)
{
#ifdef TIMING
  printf("waiting in cycle\n");
  flag = DmtcpWorker::waitForUpdateLogRequest();
  if (flag == 1)
  {
    printf("updating log now !\n");
    initMap(checkpoint_location);
    iter = checkpoint_location.begin();
    std::string API_name = iter->second.first;
    int exe_count = iter->second.second + cycle * (api_count[API_name]);
    printf("name is : %s\n", API_name.c_str());
    printf("exe count is : %d\n", exe_count);
    ThreadSync::setFlag((char *) API_name.c_str(), exe_count);
    iter++;
    flag = -1;
  }
  else if (flag == 2)
  {
    ...
    clock_t end1 = clock();
    double elapsed1 = (end1 - start1) / (double) CLOCKS_PER_SEC;
    printf("time cost to checkpoint file is : %f\n", elapsed1);

    // get next checkpoint location
    if (iter == checkpoint_location.end())
    {
      cycle++;
      iter = checkpoint_location.begin();
    }
  ...
  {
    printf("user change interval now\n");
    initMap(checkpoint_location);
    iter = checkpoint_location.begin();
    cycle++;
    std::string API_name = iter->second.first;
...
ThreadList::suspendThreads()
{
  ...
  JTRACE("before suspendThreads get lock..0");
  DmtcpRWLockInit(&threadResumeLock);
  JTRACE("before suspendThreads get lock..1");
  JASSERT(DmtcpRWLockWrLock(&threadResumeLock) == 0);
  JTRACE("before suspendThreads get lock..2");
...
stopthisthread(int signum)
{
  ...
  if (Thread_UpdateState(curThread, ST_SUSPINPROG, ST_SIGNALED)) {
    ...
    JTRACE("User thread returning to user code")
      (curThread->tid) (__builtin_return_address(0))(szThreadName);
  }
}
...
ThreadList::waitForAllRestored(Thread *thread)
{
  if (thread == ckptThread) {
    JTRACE("Ckpt Thread...")(numUserThreads);
    int i;
    ...
    JTRACE("No Ckpt Thread");
    sem_post(&semNotifyCkptThread);
    sem_wait(&semWaitForCkptThreadSignal);
    JTRACE("None ckpt thread continue restore...");
  }
  ...
  JTRACE("waitForAllRestored finished....");
}
...
ThreadList::postRestart(double readTime)
{
  Thread *thread;
  sigset_t tmp;
  JTRACE("ThreadList::postRestart...0");
  ...
  JTRACE("ThreadList::postRestart...0.1");
  SharedData::postRestart();

  JTRACE("ThreadList::postRestart...0.2");
  ...
  for (thread = activeThreads; thread != NULL; thread = thread->next) {
    ...
    JTRACE("ThreadList::postRestart...0.3");
  ...
  JTRACE("ThreadList::postRestart...1");
...
{
  JTRACE("ThreadList::restarthread...0");
