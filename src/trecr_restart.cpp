...
static const char *theUsage =
  ...
  "  --restartdir\n"
  "              The directory of checkpointing, the default directory is current directory.\n"
  "  --help\n"
...
static void setEnvironFd();
static void runMtcpRestart(int fd, ProcessInfo *pInfo, bool isKernelloaderApp);
...
RestoreTarget::createProcess(bool createIndependentRootProcesses)
{
  ...
  runMtcpRestart(_fd, &_pInfo, isKernelLoaderApp());

  JASSERT(false).Text("unreachable");
}

bool
RestoreTarget::isKernelLoaderApp()
{
  return strstr(_path.c_str(), "ckpt_kernel-loader.exe");
}
...
static void
runMtcpRestart(int fd, ProcessInfo *pInfo, bool isKernelloaderApp)
{
  ...
  if (runMtcpSplitProcess && isKernelloaderApp) {
    newArgs.push_back((char *)kernelLoader.c_str());
    newArgs.push_back((char *)"--restore");
    newArgs.push_back((char *)fdBuf.c_str());
  } else {
    mtcp_restart = Util::getPath("mtcp_restart");
    pause_param = get_pause_param();
    stderrFd = jalib::XToString(PROTECTED_STDERR_FD);

    newArgs.push_back((char *)mtcp_restart.c_str());
    newArgs.push_back((char *)"--fd");
    newArgs.push_back((char *)fdBuf.c_str());
    newArgs.push_back((char *)"--stderr-fd");
    newArgs.push_back((char *) stderrFd.c_str());
    if (pause_param) {
        newArgs.push_back((char *) "--mtcp-restart-pause");
        newArgs.push_back(pause_param);
    }
  }
  ...
  // execve("/home/xingbiao/DMTCP/CRAC-early-development-master/contrib/split-cuda/kernel-loader.exe", &newArgs[0], environ);
...
open_ckpt_to_read(const char *filename)
{
  ...
  /*********************by tian01.liu 2022.8.18********************************/
  // bitcomp decompress the dmtcp file
  else if (fc == 0) {
    // printf("[LT] enter bitcomp handling...");
    const char* bitcomp_path = const_cast<char*>("/home/tian01.liu/nvComp/nvcomp/bin/benchmark_hlif");
    static const char *bitcomp_args[] = {
        const_cast<char *>("/home/tian01.liu/nvComp/nvcomp/bin/benchmark_hlif"),
        const_cast<char *>("bitcomp"),
        const_cast<char *>("--decompress"),
        NULL
    };

    decomp_path = bitcomp_path;
    decomp_args = bitcomp_args;
...
processArgs(int *orig_argc, const char ***orig_argv)
{
  ...
  while (argc > 0) { // ... -restartdir ./ OR ... ckptImg
    ...
    if (s == "--help" && argc == 1) {
      ...
      setenv(ENV_VAR_MPI_SUPPORTED, "1", 1);
      shift;
    } else if (s == "-q" || s == "--quiet") {
...
main(int argc, const char **argv)
{
  ...
  clock_t time_clk_0 = clock();
  // printf("[LT] start decompress time:%ld\n", time_clk_0);
  for (; argc > 0; shift) {
    string restorename(argv[0]);
    struct stat buf;
    JTRACE("File doesn't have .dmtcp extension. Check Usage.") (restorename);
  ...
  if (restartDir.empty() ^ (ckptImages.size() > 0)) {
    JNOTE("File doesn't have .dmtcp extension. Check Usage.") (restartDir)(ckptImages);
    JASSERT_STDERR << theUsage;
    exit(DMTCP_FAIL_RC);
  }
  #endif
  ...
  if (getenv(ENV_VAR_MPI_SUPPORTED) != nullptr &&
      strcmp(getenv(ENV_VAR_MPI_SUPPORTED), "1") == 0) {
      if (dmtcp_restart_plugin != NULL) {
        dmtcp_restart_plugin(restartDir, ckptImages);
      }
  }
  JNOTE("New dmtcp_restart process checkpoint images");
...
processCkptImages()
{
  ...
  printf("[DMTCP_RESTART] foundNonOrphan = %d\n", foundNonOrphan);
  foundNonOrphan = false;
  if (foundNonOrphan) {
